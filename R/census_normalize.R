...
census_normalize <- function(
  ...
  present_vars <- unique(unlist(
    lapply(
      interpolated[names(interpolated) %in% census_scales],
      function(scale_list) {
        unlist(
    ...
    function(scale) {
      sapply(
        ...
        function(year) {
          ...
          pct_vars <- intersect(
            names(data),
            units$var_code[units$units == "Percentage"]
          )
          ...
          numb_vars <- intersect(
            names(data),
            units$var_code[units$units == "Number"]
          )
          if (length(numb_vars) > 0) {
            numb_list <- lapply(numb_vars, function(x) {
              ...
              if (
                length(parent_string) == 0L ||
                  !parent_string %in% names(data_no_geo)
              ) {
          ...
          keep_cols <- !names(data) %in%
            names(pcts_numb)[names(pcts_numb) != "ID"]
