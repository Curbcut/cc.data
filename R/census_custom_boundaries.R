...
census_custom_boundaries <- function(destination, data_raw_DA,
  ...
  normalized <- census_normalize(interpolated, census_scales = names(interpolated))
  parent_dropped <- census_drop_parents(normalized)
  census_reduce_years(parent_dropped, census_scales = names(interpolated))
