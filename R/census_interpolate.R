...
census_interpolate <- function(
  ...
  present_vars <- unique(unlist(
    lapply(data_raw[names(data_raw) %in% census_scales], function(scale_list) {
      unlist(
        lapply(scale_list, function(df) {
          setdiff(names(df), c("ID", "geometry"))
        }),
        use.names = FALSE
      )
    })
    ...
    function(scale) {
      future.apply::future_sapply(
        ...
        function(year) {
          ...
          int <- int[
            sf::st_is(int, "POLYGON") | sf::st_is(int, "MULTIPOLYGON"),
          ]
          ...
          if (!nrow(int)) {
            ...
            res <- tibble::as_tibble(merge(
              out,
              all_dest[c("ID", "geometry")]
            )) |>
              sf::st_as_sf()
            pb()
            return(res)
          ...
          if (length(add)) {
            ...
            add_vars <- tibble::tibble(cbind(
              int[!names(int) %in% names(add_vars)],
              add_vars
            ))
          } else {
          ...
          if (length(avg)) {
            avg_vars <- lapply(avg, function(x) {
              ids <- lapply(unique(int$ID), function(id) {
                ...
                val <- if (
                  length(parent_string) == 0L || !parent_string %in% names(dat)
                ) {
                  NA_real_
                } else {
                  cc.data::weighted_mean(dat[[x]], dat[[parent_string]])
                }
                na_pct <- sum(is.na(dat[[x]]) * dat$int_area) /
                  sum(dat$int_area)
                if (is.nan(na_pct) || na_pct >= 0.5) {
                  val <- NA_real_
                }
                out <- tibble::tibble(ID = id, val = val)
                names(out)[2] <- x
                out
              })
              tibble::as_tibble(Reduce(rbind, ids))
            })
            avg_vars <- tibble::as_tibble(Reduce(
          ...
          if (length(add)) {
            ...
            add_final <- tibble::as_tibble(Reduce(
              function(a, b) merge(a, b, by = "ID"),
              add_final
            ))
          } else {
          ...
          non_interp <- sf::st_drop_geometry(y_df[
            is.na(match(y_df$ID, out$ID)),
          ])
